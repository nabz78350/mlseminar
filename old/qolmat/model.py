...
class TabDDPM:
    ...
    def _q_sample(self, x: torch.Tensor, t: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        standard_normal_samples = np.random.normal(size=(n_obs, n_features))
        samples_transformed = standard_normal_samples.dot(L.T)
        epsilon = torch.tensor(samples_transformed)
    ...
    def _process_data(
        ...
        if is_training:
            if self.is_rolling:
                ...
                for x_w in x.resample(rule='1B', level=self.index_datetime):
        ...
        for x_w in x_windows:
            ...
            # print(x_w_fillna.values)
            # print(x_w_fillna.values.shape)
            assert x_w.shape[0] >0 
