...
class TSGM(nn.Module):
    ...
    def sample(self,
        ...
        h_start = self.RNN.encoder(start)
        h_start = h_start[:, -1, :]
        print(h_start.shape)
        #h_start = start.unsqueeze(1)

        h = pc_sampler(h_start.unsqueeze(1),  # shape: (batch_size, 1, dim_input)
                       self.score_model, sde,
                       shape,
                       predictor, corrector,
                       snr, self.device, n_steps=n_steps)

        x = self.RNN.predict(start.squeeze(1), h)  # shape: (batch_size, window_size, dim_input)
        if return_h:
            return x, h
        return x
